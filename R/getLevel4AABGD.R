...
#'close(level1b)
#'@export
getLevel4AAGBD<-function(level4a, select=c("elev_lowestmode")) {
  
  select <- unique(c("lat_lowestmode", "lon_lowestmode","shot_number", "agbd", "agbd_pi_lower", "agbd_pi_upper", "agbd_se", select))
  level4a <- level4a@h5
  
  datasets <- hdf5r::list.datasets(level4a, recursive = T)
  ...
  for ( i in dtse2){
    ...
    if ( name_i =="shot_number"){
      assign(name_i, bit64::c.integer64(get(name_i),level4a[[i]][]))
    } else {
      assign(name_i, c(get(name_i), level4a[[i]][]))
    }
  }
  
  level4a.dt<-data.table::data.table(as.data.frame(get("shot_number")[-1]))
  ...
  colnames(level4a.dt)<-c("shot_number",select2)
  close(pb)
  return(leve4a.dt)
}
